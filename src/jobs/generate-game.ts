...
async function run() {
	try {
        ...
        if (!protondbData && !steamdeckhqData && !sharedeckData) {
            logger.warn(`No scraped data found for game ${gameInQueue.game_id}. Cannot generate entry.`);
...
async function generateGameEntry(
	...
	const data = protonMiner.extractData(protondbData.scraped_content);

	// Here you would typically process the extracted data and save it to your database.
	// This is a placeholder for the actual implementation.
	console.log(`Generated data for game ${game_id}:`, data);
}

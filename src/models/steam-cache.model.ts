...
import type { SteamSearchCache, SteamGameDetailsCache } from "../schemas/steam-cache.schema";
import type { SteamSearch, SteamApp } from "../services/steam/steam.types";
...
export const getCachedSearchResults = async (term: string, limit: number) => {
	...
	const cached = await db.collection<SteamSearchCache>(SEARCH_CACHE_COLLECTION).findOne({
		term,
		limit,
		expires_at: { $gt: now },
	});
 ...
 */
export const saveSearchResultsCache = async (term: string, limit: number, data: SteamSearch) => {
	const db = getDB();
	const now = new Date();
...
export const getCachedGameDetails = async (gameId: number) => {
	...
	const cached = await db.collection<SteamGameDetailsCache>(DETAILS_CACHE_COLLECTION).findOne({
		game_id: gameId,
		expires_at: { $gt: now },
	});
...
export const saveGameDetailsCache = async (gameId: number, data: SteamApp) => {
	...
	await db.collection<SteamGameDetailsCache>(DETAILS_CACHE_COLLECTION).updateOne(
		{ game_id: gameId },
		{
			$set: {
				data,
...
export const createCacheIndexes = async () => {
	...
	await db.collection<SteamSearchCache>(SEARCH_CACHE_COLLECTION).createIndex(
		{ expires_at: 1 },
	...
	await db.collection<SteamGameDetailsCache>(DETAILS_CACHE_COLLECTION).createIndex(
		{ expires_at: 1 },
		{ expireAfterSeconds: 0 },
	);

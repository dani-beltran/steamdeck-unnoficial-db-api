...
if (!process.env.STEAM_API_KEY) {
    console.error("Error: STEAM_API_KEY environment variable is not set. Exiting application.");
    process.exit(1);
}
if (!process.env.CLAUDE_API_KEY) {
    console.error("Error: CLAUDE_API_KEY environment variable is not set. Exiting application.");
    process.exit(1);
}
if (!process.env.SESSION_SECRET) {
    console.error("Error: SESSION_SECRET environment variable is not set. Exiting application.");
    process.exit(1);
}
